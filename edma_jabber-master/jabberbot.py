...
# Copyright (c) 2009 Marcos De Vera Piquero <marc0s@fsfe.org>
#   - Jun 2009: added some code to be able to customize connection parameters
#
...
__author__ = 'Thomas Perl <thp@thpinfo.com>\nMarcos De Vera Piquero <marc0s@fsfe.org>'
__version__ = '0.6.1'
...
class JabberBot(object):
    ...
    def __init__( self, jid, password, res = None, server = None, port = 5222, secure=False):
    ...
    def connect( self):
        if not self.conn:
            ...
            print conn,'-------',self.jid.getDomain()
            print self.server,self.port,self.secure,'------------------'
            s = conn.connect(server=(self.server, self.port), secure=self.secure)
            print s,'----------'
            if not s:
            ...
            if self.secure and s != 'ssl':
              self.log('Secure connection requested but not available.')
