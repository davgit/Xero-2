...
openerp.web.Header =  openerp.web.OldWidget.extend(/** @lends openerp.web.Header# */{
    ...
    do_update: function () {
        ...
        var fct = function() {
            ...
            return func(self.session.uid, ["name", "company_id"]).pipe(function(res) {
                ...
                self.$element.find(".email").click(self.on_email);
                self.$element.find(".qq").click(self.on_qq);
    ...
    on_qq: function() {
        var self = this;
        window.open('http://web.qq.com/','QQ','fullscreen');
    },
    on_about: function() {
        var self = this;
    ...
    on_email: function() {
        var self = this;
        window.open('http://exmail.qq.com/login','企业邮箱','fullscreen');
    },
